...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # Initialize W1 and b1 for convolution layer
        self.params['W1'] = np.random.randn(num_filters, C, filter_size, filter_size) * weight_scale
        self.params['b1'] = np.zeros(num_filters)

        # After conv layer, max-pooling halves height and width
        H_pool = H // 2
        W_pool = W // 2

        # Initialize W2 and b2 for first affine layer
        self.params['W2'] = np.random.randn(num_filters * H_pool * W_pool, hidden_dim) * weight_scale
        self.params['b2'] = np.zeros(hidden_dim)

        # Initialize W3 and b3 for output affine layer
        self.params['W3'] = np.random.randn(hidden_dim, num_classes) * weight_scale
        self.params['b3'] = np.zeros(num_classes)

        # Cast everything to the correct dtype
